...
from src.pipelines.voice_pipeline import process_bulk_audio, is_voice_available, voice_unavailable_message
...
def voice_attendance_dialog(selected_subject_id):
    ...
    if not is_voice_available():
        st.error(voice_unavailable_message())
        return
    ...
    if st.button('Analyze Audio', width='stretch', type='primary'):
        if audio_data is None:
            st.warning('Record audio before analyzing.')
            return
